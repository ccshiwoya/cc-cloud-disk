...
void MyFileWg::addUploadFiles()
{
    ...
    //getOpenFileNames获取到多个要上传的文件的绝对路径
...
void MyFileWg::uploadFile(UploadFileInfo *info)
{
    ...
    */
    ...
    data.append( QString("user=\"%1\" ").arg( login->getUser() ).toLocal8Bit() ); //上传用户
    data.append( QString("filename=\"%1\" ").arg(fileName).toLocal8Bit() ); //文件名字
    data.append( QString("md5=\"%1\" ").arg(md5).toLocal8Bit() ); //文件md5码
    data.append( QString("size=%1").arg(size).toLocal8Bit()  );   //文件大小
    data.append("\r\n");
