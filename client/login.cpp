    ...
    ui(new Ui::Login)
{
...
#if 1
    // 测试数据
    ui->reg_usr->setText("chengchong");
    ui->reg_nickname->setText("chengchong");
    ui->reg_pwd->setText("123456");
    ui->reg_surepwd->setText("123456");
    ui->reg_phone->setText("13955992635");
    ui->reg_mail->setText("1758783808@qq.com");
    ...
    {
        // 切换到设置界面，通过stackedWidget
        //两种方法，setCurrentndex（参数为编号）和setCurrentWidget（参数为指针）
        ui->stackedWidget->setCurrentWidget(ui->set_page);
        ui->address_server->setFocus();
    });

    // 关闭按钮，看当前窗口是什么，currentWidget()获取当前窗口，setCurrentWidget()切换窗口
...
// 得到服务器回复的登陆状态， 状态码返回值为 "000", 或 "001"，还有登陆token
...
void Login::on_login_btn_clicked()
{
    ...
    // 接收服务器发回的http响应消息,等待finished或者readyRead信号
...
void Login::on_register_btn_clicked()
{
    ...
    // 设置请求
    QNetworkRequest request;
    // 设置连接服务器要发送的url，"/reg"就是服务端要处理的指令
    ...
    request.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("application/json"));
    request.setHeader(QNetworkRequest::ContentLengthHeader, QVariant(array.size()));
    // 发送数据，并保存响应数据，m_manager为唯一的处理网络请求类对象
    QNetworkReply* reply = m_manager->post(request, array);
