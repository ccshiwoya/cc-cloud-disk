...
void Common::moveToCenter(QWidget *tmp)
{
     ...
     // 屏幕中间显示
     // 使用qApp->desktop();也可以
     //QApplication唯一应用类，desktop()获取当前屏幕高度和宽度
...
void Common::writeLoginInfo(QString user, QString pwd, bool isRemeber, QString path)
{
    ...
    // type_path信息
    //m_typePath是静态变量，在最上方已经初始化为FILETYPEDIR
    ...
    // 也可以用toLocal8Bit(), 转换为本地字符集，如果windows则为gbk编码，如果linux则为utf-8编码
    ...
    //通过fromVariant()把Qmap对象转化为document
...
void Common::writeWebInfo(QString ip, QString port, QString path)
{
    ...
    //至此，所有配置文件的数据都保存在json里，接下来添加到配置文
...
QString Common::getFileMd5(QString filePath)
{
    ...
    quint64 totalBytes = 0; //总共要写
    quint64 bytesWritten = 0; //已写
    quint64 bytesToWrite = 0; //未写
    quint64 loadSize = 1024 * 4; //每次写入大小
    QByteArray buf;

    totalBytes = localFile.size();
    bytesToWrite = totalBytes;

    //每次读load ize = 4k 大小，然后加到ch上
    while (1)
    {
        if(bytesToWrite > 0)
        {
...
// 将某个字符串加密成md5码，可以直接一次性加密完成
QString Common::getStrMd5(QString str)
{
    QByteArray array;
    //md5加密，直接调用hash（第一个参数类型为QByteArray，需要转换，也可以调用toutf8()）
